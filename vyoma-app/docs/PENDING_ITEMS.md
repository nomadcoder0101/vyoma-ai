...
This list reflects the current live app state after Postgres storage, first-party login, route protection, and Vercel deployment.
...
| --- | --- | --- | --- |
| P0 | Managed authentication provider | Pending | Choose and configure Clerk/Auth.js keys. Current first-party session protects the pilot but does not verify email ownership. |
...
| P1 | OpenAI production assistant key | Pending | Add `OPENAI_API_KEY` in Vercel when ready. Local fallback currently remains available. |
...
| P2 | Production smoke test | Done | `npm.cmd run smoke:live` checks public pages, anonymous route blocking, redirect behavior, login cookie, and signed-in tracker API. |
